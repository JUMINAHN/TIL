        ...
        for col in range(N):
            if binggo[row][col] == talk_num[idx]: #같다면 -> 오타
                ...
                if row_col_sum + leftright + rightleft >= K: #3 이상일 때
                    print(idx)
                    idx = N*N*N #while문 자체를 종료시키기 위함


                    # @유의할 점@
                    # idx = N*N*N으로 while 루프를 종료하려는 의도가 있지만,
                    # 이 break가 전체 while 루프를 빠져나가는 게 아니므로 원하는 효과를 내지 못할 수 있음.

                    break #for col문 break
                    # #한 행(row)만 탈출하게 돼서 나머지 루프를 중단하지 않음.
